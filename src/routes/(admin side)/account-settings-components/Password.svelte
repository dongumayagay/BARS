...
{#if !enableEdit}
<section class="w-[80%] lg:w-1/2 h-max flex flex-col gap-4 bg-neutral p-4 z-100 rounded-xl shadow-xl" class:hidden={otherModalOpened}>
    <button class="flex justify-between border-b-2 border-transparent hover:border-inherit group p-2" on:click={()=>{enableEdit=true; dispatch("open")}}>
        <p class="w-1/3 text-start font-semibold">Change Password</p>
        <!-- <p class="w-full">Click EDIT to change</p> -->
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-100">
