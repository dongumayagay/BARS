...
<div class="w-screen h-screen fixed top-0 left-0 flex flex-col items-center justify-center bg-black/70 z-20" on:click={()=>dispatch("close")}>
    ...
    <div class="carousel flex items-center justify-start gap-4 w-full">
        <div class="absolute px-[3%] w-full flex justify-between z-20">
            <button class="hidden btn btn-circle btn-primary md:block z-20" disabled={filePaths.length === 1} on:click|stopPropagation={()=>slideHandler("left")}>❮</button> 
            <button class="hidden btn btn-circle btn-primary md:block z-20" disabled={filePaths.length === 1} on:click|stopPropagation={()=>slideHandler("right")}>❯</button>
        </div>
        ...
        <div class="w-full h-screen flex items-center justify-center p-4">
            <div id={"slide" + index} class="carousel-item flex justify-center relative w-screen max-h-[80%] z-1" on:click|stopPropagation={()=>{}} use:swipe={{ timeframe: 1000, minSwipeDistance: 100, touchAction: 'pan-y' }} on:swipe={swipeHandler}>
                <img src={url} alt={url} class="w-max max-w-[80%]" use:zoom={1.05}  />
            </div> 
        </div>
