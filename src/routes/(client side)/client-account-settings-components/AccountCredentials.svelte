...
{#if !enableEdit}
    <section class="w-[80%] lg:w-1/2 h-max flex flex-col gap-4 bg-neutral p-4 rounded-xl shadow-lg z-20" class:hidden={otherModalOpened}>
        ...
        <div class="flex flex-col lg:flex-row justify-center border-b-2 border-transparent hover:border-inherit group p-2">
            <p class="w-full lg:w-1/3 text-start font-semibold">Username: </p>
            <p class="w-full text-center lg:text-start p-4 lg:p-0 {!!$userStore.displayName ? "" : "opacity-70"}">{$userStore.displayName??"Click EDIT to add username"}</p>
        </div>
        <div class="flex flex-col lg:flex-row justify-center border-b-2 border-transparent hover:border-inherit group p-2">
            <p class="w-full lg:w-1/3 lg:text-start font-semibold">Email Address: </p>
            <p class="w-full text-center lg:text-start p-4 lg:p-0">{$userStore.email}</p>
        </div>
    </section>
{/if}
{#if enableEdit}
