<script>
    ...
    export let signUpStep, emailIsUsed;
...
<form class="absolute w-[85vw] min-h-[70vh] lg:w-[40vw] h-max bg-neutral flex flex-col rounded-lg shadow-xl p-6" on:submit|preventDefault={submitHandler} class:hidden={signUpStep!==0} >
    ...
    <div class="w-full h-full flex flex-col gap-4">
        <div class="w-full flex flex-col border-b-2 border-primary gap-2">
            <label for="#username" class="font-semibold label-text">Username</label>
        ...
        <div class="w-full flex flex-col border-b-2 border-primary gap-2">
            <label for="#email" class="font-semibold label-text">Email</label>
            <input type="email" id="email" class="input {emailIsUsed ? "input-error" : ""} bg-transparent focus:bg-transparent focus:outline-0 autofill:bg-transparent" placeholder="example@provider.com" bind:value={email}>
        </div>
        <label for="#email" class="label-text-alt text-error font-semibold" class:hidden={!emailIsUsed}>This email address has been already used</label>
        <div class="flex flex-col gap-2">
            <div class="w-full flex flex-col border-b-2 border-primary gap-2">
                <label for="#password" class="font-semibold label-text">Password</label>
        ...
        <div class="flex flex-col gap-2">
            <div class="w-full flex flex-col border-b-2 border-primary gap-2">
                <label for="#password" class="font-semibold label-text">Re-type Password</label>
