...
<form on:submit|preventDefault={dispatchHandler} class="w-full h-max flex flex-col lg:flex-row items-center gap-4">
    ...
    <div class="w-full flex gap-3">
        <input title="Send a message" placeholder="Send a message" class="input input-bordered input-sm input-accent w-full " bind:value={content} disabled={status==="Request Completed" || status==="Appointment Served"} required>
    ...
    <div class="w-full flex gap-3">
        ...
        />
        <button type="submit" title="Send message" class="lg:hidden" disabled={status==="Request Completed" || status==="Appointment Served"}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" />
            </svg>          
        </button>
    ...
    <select class="w-[] lg:w-[25%] select select-error dark:select-accent select-sm max-w-xs" bind:value={messageType} disabled={status==="Request Completed" || status==="Appointment Served"}>
        <option value="text" selected>Send a Message</option>
        <option value="file">Upload a File</option>
    </select>

    <button type="submit" title="Send message" class="hidden lg:block" disabled={status==="Request Completed" || status==="Appointment Served"}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" />
        </svg>          
    </button>
